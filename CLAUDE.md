...
| Tarih | Yapılan | Sonuç |
|-------|---------|-------|
| 5 Ağu (2) | **📦 GIT SYNC + README:** 4–5 Ağu'nun commit'siz duran tüm işi commit edildi (`cf31002`: takvim, planner revizyonu, pencere odaklama, OCR fiksleri, koyu başlık çubuğu — 26 dosya / +2579 satır) ve `origin/main`'e pushlandı. `README.md` v3.0 durumuna göre baştan yazıldı (`d01897d`): yetenek listesi, 14 katmanlı mimari, AIP fallback zinciri, config tablosu, test talimatı. **NOT: CLAUDE.md ↔ AGENTS.md ikizleri ayrıştı** — AGENTS.md'de olan OCR/Takvim bölümleri ve 4 Ağu satırı CLAUDE.md'de yok; bir sonraki oturumda senkronlanmalı | ✅ Repo temiz, 631 test yeşil |
| 5 Ağu | **📅 TAKVİM, PLAN ONAY, PENCERE ODAK & OCR FİKSİ:** `features/calendar_tools.py` — yerel `takvim_etkinlikleri` DB tablosu + keysiz iCal/ICS aboneliği + 30dk arka plan senkronu. Takvim başlığında 'günü'/'arası' temizleme fiksi. **Plan Onay Fiksi:** "izin veriyorum"/"izin ver" onay kelimelerine (`_ONAY_KELIMELERI` / `CONFIRM_PHRASES`) eklendi, plan askıda kalmıyor. **🖥️ Pencere Odaklama Motoru (`core/world_state.py`):** `acik_pencereleri_listele`, `pencereyi_one_getir`, `uygun_pencereyi_odakla` eklendi. Girdideki uygulama/sekme ipucuna göre doğru pencereye otomatik odaklanır. **👁️ Vision OCR Fiksi:** `metni_bul` ve `tiklama_hedefi_coz` güncellendi — yönelme ekleri ('1'yi aç' → '1'e'/'1') ve rakamlı hedeflerde çoklu eşleşmede ilk adaya (`sira=0`) tıklama kuralı eklendi. Windows koyu başlık çubuğu (`_koyu_baslik_cubugu` DWM API) + `AICoreWidget` tema paletleri. `KOMUTLAR.md` güncellemesi | ✅ **630 test yeşil** (20 sn) |
